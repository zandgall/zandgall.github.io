...
var kode = {
    ...
    set: function() {
	    ...
	    kode.space=keys[32] || (mouseClicked && (mouseX<can.width-290 || CURRENTMENU==null));
...
function draw(){
    ...
    if(kode.space) {
        towersmenu.setOn(col(mouseX, mouseY, 10, 10, 450, can.height-30, 100, 30));
    }
    ...
    if(kode.space) {
        if(TOWERPLACE!=-1){
            switch(TOWERPLACE) {
                case 0:
                    cannon0s[cannon0s.length] = new Cannon0(mouseX, mouseY);
                    TOWERPLACE=-1;
                    money-=200;
                    break;
                case 1:
                    cannon1s[cannon1s.length] = new Cannon1(mouseX, mouseY);
                    TOWERPLACE=-1;
                    money-=500;
                    break;
            }
