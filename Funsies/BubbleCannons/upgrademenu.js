...
class UpgradeMenu {
    ...
    tick() {
        ...
        for(var i = 0; i<this.names.length; i++) {
            if(mouseX>can.width-290 && mouseX<can.width && mouseY>0 && mouseY<can.height) {
                if(mouseY>i*50+20 && mouseY<i*50+70 && i==this.finished){
...
class CannonMenus {
    ...
    tick() {
        ...
        for(var i = 0; i<this.names.length; i++) {
            if(mouseX>can.width-290 && mouseX<can.width && mouseY>0 && mouseY<can.height) {
                if(mouseY>i*100+20 && mouseY<i*100+110){
                    this.finished=i;
                    if(mouseClicked && this.buy(money)){
                        return i;
                    }
        ...
        if(TOWERPLACE!=-1)
            switch(TOWERPLACE){
                case 0:
                    ...
                    c.beginPath();
                    c.ellipse(mouseX, mouseY, 450, 450, 0, 0, 180, false);
                    var ca = new Cannon0(mouseX, mouseY);
                    ca.tick();
                    ca.ren(c);
                    break;
                case 1:
                    ...
                    c.beginPath();
                    c.ellipse(mouseX, mouseY, 450, 450, 0, 0, 180, false);
                    var ca = new Cannon1(mouseX, mouseY);
                    ca.tick();
                    ca.ren(c);
                    break;
                             }
